...
public:
	...
	TArray<struct FScriptDelegate>                     SpeechRecognitionCompleteDelegates;            // 0x0410 (0x0010) [0x0000000000400000] (CPF_NeedCtorLink)
	...
	TArray<struct FName>                               UnlockedDLC;                                   // 0x04A8 (0x0010) [0x0000000000402000] (CPF_Transient | CPF_NeedCtorLink)
	TArray<struct FScriptDelegate>                     UnlockedDLCDelegates;                          // 0x04B8 (0x0010) [0x0000000000402000] (CPF_Transient | CPF_NeedCtorLink)
	TArray<struct FScriptDelegate>                     ExternalUIChangeDelegates;                     // 0x04C8 (0x0010) [0x0000000000402000] (CPF_Transient | CPF_NeedCtorLink)
...
public:
	...
	void OpenPS4DisplayMode(uint8_t LocalUserNum, EPS4DisplayMode DisplayMode, TArray<class FString> Targets, int32_t ServiceLabel);
...
public:
	...
	bool ReceiveMessage(class FString& EncodedMessage, struct FUniqueNetId& PlayerID);
	bool EndSession();
	bool BeginSession(float RegisterTimeoutSeconds);
	void Init();
	void EventMessageGenerated(class FString EncodedMessage, struct FUniqueNetId PlayerID);
};
...
public:
	...
	void ReportPlayer(class FString& ProductUserId);
	bool ReceiveMessage(class FString& EncodedMessage, struct FUniqueNetId& PlayerID);
	bool IsAntiCheatEnabled();
	bool EndSession();
	bool BeginSession(float RegisterTimeoutSeconds);
	void Init();
	void EventPlayerReportComplete(class FString ProductUserId, bool bSuccess);
	void EventClientIntegrityViolation(EClientIntegrityViolation Violation, class FString Message);
};
...
public:
	...
	bool ReceiveMessage(class FString& EncodedMessage, struct FUniqueNetId& PlayerID);
	bool UnregisterClient(struct FUniqueNetId& PlayerID);
	bool RegisterClient(bool bProtected, struct FUniqueNetId& PlayerID, class FString& ProductUserId);
	bool EndSession();
	bool BeginSession(float RegisterTimeoutSeconds);
	void Init();
	void EventClientAuthStatusChanged(struct FUniqueNetId PlayerID, EClientAuthStatus AuthStatus);
...
public:
	...
	bool UpdateOnlineGame(struct FName SessionName, class UOnlineGameSettings* UpdatedGameSettings, bool bShouldRefreshOnlineData);
};
...
public:
	...
	bool ReadAchievements(uint8_t LocalUserNum, int32_t TitleId, bool bShouldReadText, bool bShouldReadImages);
	EOnlineEnumerationReadState GetAchievements(uint8_t LocalUserNum, int32_t TitleId, TArray<struct FAchievementDetails>& Achievements);
	...
	bool ShowDeviceSelectionUI(uint8_t LocalUserNum, int32_t SizeNeeded, bool bForceShowUI);
	...
	bool CanCommunicateVoice(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanCommunicateVideo(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanCommunicateText(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanShareUserCreatedContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanAccessPremiumVideoContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanAccessPremiumContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanUseCloudStorage(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanRecordDVRClips(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanBrowseInternet(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanShareWithSocialNetwork(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanShareKinectContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanUploadFitnessData(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	...
	bool ShowKeyboardUI(uint8_t LocalUserNum, class FString TitleText, class FString DescriptionText, bool bIsPassword, bool bShouldValidate, class FString DefaultText, int32_t MaxResultLength);
	...
	EOnlineEnumerationReadState GetFriendsList(uint8_t LocalUserNum, int32_t Count, int32_t StartingAt, TArray<struct FOnlineFriend>& Friends);
	...
	bool CanShowPresenceInformation(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanViewPlayerProfiles(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanPurchaseContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	bool CanDownloadUserContent(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	EFeaturePrivilegeLevel CanCommunicate(uint8_t LocalUserNum, ECommunicationMethod CommMethod, bool bAttemptToResolve);
	bool CanPlayOnline(uint8_t LocalUserNum, bool bAttemptToResolve, class FString Reason, EFeaturePrivilegeLevel& PrivilegeLevelHint);
	...
	bool AutoLogin();
	bool Login(uint8_t LocalUserNum, class FString LoginName, class FString Password, bool bWantsLocalOnly);
	bool ShowLoginUI(uint8_t LocalUserNum, bool bShowOnlineOnly);
...
public:
	...
	void ClearPlayerTalkingDelegate(struct FScriptDelegate TalkerDelegate);
	void AddPlayerTalkingDelegate(struct FScriptDelegate TalkerDelegate);
	bool UnmuteRemoteTalker(uint8_t LocalUserNum, struct FUniqueNetId PlayerID, bool bIsSystemWide);
	bool MuteRemoteTalker(uint8_t LocalUserNum, struct FUniqueNetId PlayerID, bool bIsSystemWide);
	bool SetRemoteTalkerPriority(uint8_t LocalUserNum, struct FUniqueNetId PlayerID, int32_t Priority);
	bool IsHeadsetPresent(uint8_t LocalUserNum);
	bool IsRemotePlayerTalking(struct FUniqueNetId PlayerID);
	bool IsLocalPlayerTalking(uint8_t LocalUserNum);
